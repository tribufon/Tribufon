...
@objc class CallManager: NSObject {
    ...
    func debug() -> String {
        return lc?.calls.compactMap { $0.callLog?.callId }.joined(separator: "|") ?? "no calls"
    }
	...
	@objc func displayIncomingCall(callId: String) {
		...
		if (uuid != nil) {
			...
			if (callInfo?.declined ?? false) {
                ...
                providerDelegate.reportIncomingCall(call:nil, uuid: uuid!, handle: uuid!.uuidString + " - uuid", hasVideo: false)
				providerDelegate.endCall(uuid: uuid!)
			}
			return
		}
        
        let calls = CallManager.instance().debug()
        print("calls: \(calls)")
		...
		if (call != nil) {
			...
			displayIncomingCall(call: call, handle: callId + " - exists", hasVideo: video, callId: callId)
		} else {
            displayIncomingCall(call: nil, handle: "call id: " + (callId.isEmpty ? "empty" : callId), hasVideo: false, callId: callId)
		}
	}
		...
		switch cstate {
			case .IncomingReceived:
				if (CallManager.callKitEnabled()) {
					...
					content.sound = UNNotificationSound.init(named: UNNotificationSoundName.init("peek_door_quest.caf"))
